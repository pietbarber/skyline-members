...
if (param('uploaded_file')) {
  ...
  while (my $line = <$fh>) {
	...
	my ($lastname, $firstname, $amount, $middleinitial, $namesuffix);
 	...
 	if ($line =~ /"([A-Za-z]+), ([A-Za-z]+)",(-?[0-9-.]+)/) {
		$lastname=$1; 
		$firstname=$2; 
		$amount=$3;
		}

 	elsif ($line =~ /"([A-Za-z]+), ([A-Za-z]+) ([A-Za-z\.]+)",(-?[0-9-.]+)/) {
		$lastname=$1; 
		$firstname=$2; 
		$middleinitial=$3;
		$amount=$4;
...
treasurer\@skylinesoaring.org, or by phone: 434 979-5289
...
c/o Dan Noonan<br>
660 Gillums Ridge Rd <br>
Charlottesville, VA 22903</tt><br>
      <br>
If you have any issues with the above balance, please contact the club
treasurer by email to: <br>

<a class="moz-txt-link-abbreviated" href="mailto:treasurer\@skylinesoaring.org">treasurer\@skylinesoaring.org</a>, or by phone: 434 979-5289<br>
      <br>
      </td>
    </tr>
  </tbody>
...
sub email_balance {
  ...
  my $invoice =<<EOM;
From: "Skyline Soaring Club Treasurer" <treasurer\@skylinesoaring.org>
To: "$firstname $lastname" <pbarber\@skylinesoaring.org>
X-Accept-Language: en-us, en
MIME-Version: 1.0
