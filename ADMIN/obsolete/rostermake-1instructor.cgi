...
main: {
  ...
  if ($q->param('Commit All Changes to Duty Roster') ||
    ...
    print "<!-- making wap page -->\n";
    open (WAPOUT, ">/var/www/skyline/html/WAP/roster.html")
	|| warn ("Can't write to file for WAP $!\n");

    open (MKWAP, "-|") 
	|| exec ("/home/httpd/bin/mkwap.pl");
    for (<MKWAP>) {
      print WAPOUT $_;
      }
    close (MKWAP);
...
EOM
  ...
  for (sort by_num keys %roster)  {
	...
	# which column belongs to which job.
      ...
      print "<td><b>Instructor</b></td><td><b>Duty Officer</b></td>\n";
