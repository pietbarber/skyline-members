...
if (param('logsheet_contents') ne /\w/) {
  ...
  print		qq(<tr><td>Removing any pending flights..</td>\n);
  remove_pending($logsheet{'operations_date'}, $logsheet{'field'}); 
...
http://members.skylinesoaring.org/STATS/?date=$date
...
sub remove_pending {
	# Sometimes we have pending flights from instructors who want to fill in 
	# a log sheet.  In these cases, we need to run a special delete on them. 
  my ($date) = shift; 
  my ($field) = shift; 
  if (!$dbh) {
    db_connectify();
  ...
  my ($sth);
  $sth=$dbh->do(qq(delete from flight_info where glider='(pending)' and flight_date=? and field=?), undef, $date, $field);
...
sub destroy_old_info {
  ...
  my ($sth);
  $sth=$dbh->do(qq(delete from towplane_data where flight_date=? and field=?), undef, $logsheet{'operations_date'}, $logsheet{'field'});
  $sth=$dbh->do(qq(delete from flight_info where flight_date=? and field=?), undef, $logsheet{'operations_date'}, $logsheet{'field'});
  $sth=$dbh->do(qq(delete from ops_days where flight_date=? and field=?), undef, $logsheet{'operations_date'}, $logsheet{'field'});
  }
...
sub write_meta_info {
  insertify(qq(insert into ops_days (
	...
	$logsheet{'assistant'},
	$logsheet{'field'}, 
	$logsheet{'am_towpilot'}, 
	$logsheet{'pm_towpilot'}, 
	);
  }
...
sub insert_member {
  ...
  if (find_name($input->{'ContactInfo_name'}->{'PersonName'}->{'value'}) eq $input->{'ContactInfo_name'}->{'PersonName'}->{'value'}) {
    ...
    print "Test: Does $handle exist in the database? (" . handle_search($handle) . ")\n" if $DEBUG;
    if (handle_search($handle) == 0) { 
      print "Yup. Guess I need to find a handle with a number for this guy.<br>" if $DEBUG;
      for my $offset_number (1 .. 99) { 
        if (handle_search($handle . $offset_number) == 0) {
	  print "Checking for handle search for $handle + $offset_number...<br>\n" if $DEBUG; 
          $handle .= $offset_number;
          last;
          }
        }
      }
    else {
      print "Nope.  Guess we can use $handle without any number modifiers.<br>" if $DEBUG;
      }
...
weekday_list, bio_online, mugshot, public_notes, private_notes, cell_phone, emergency_contact) 
		values (?, ?, ?, ?, ?, ?, ?, ?, 
			?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 
			?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 
			?, ?, ?, ?, ?, ?, ?, ?,  
...
sub write_towplane_info {
  ...
  if (ref($input) eq "ARRAY") {
	...
	warn "New variable towplane link is: ". $towplane_link . "\n" if $DEBUG;  
        warn "I think the towplane's name is: " . $towplane_link->{'TowPlaneData_towplane'}->{'TowPlaneName'}->{'value'} . "\n" if $DEBUG; 
#                           Table "public.towplane_data"
#      Column       |         Type          |              Modifiers              
#-------------------+-----------------------+-------------------------------------
# flight_date       | date                  | not null
# towplane          | character varying(20) | default 'Pawnee'::character varying
# start_tach        | real                  | 
# stop_tach         | real                  | 
# tach_time         | real                  | 
# gas_added         | real                  | 
# towpilot_comments | character varying     | 
# tows              | integer               | 
# field             | character varying(4)  | default 'KFRR'::character varying

      insertify(qq(insert into towplane_data 
        (flight_date, towplane, start_tach, stop_tach, tach_time, gas_added,
        towpilot_comments, tows, field) values (?, ?, ?, ?, ?, ?, ?, ?, ?)),
        $logsheet{'operations_date'},
...
sub insertify {
  ...
  my ($sth);
  if ($DEBUG) {
    warn "SQL markup for insertification is: ($sql_markup)\n"; 
    }
...
sub check_for_previous {
  ...
  my ($field) = $logsheet{'field'}; 
  warn "The field is $field\n" if $DEBUG;
  warn "\$answer is $answer so far... " if $DEBUG; 
  my($get_info) = $dbh->prepare("select count(*) as quantity from flight_info where flight_date ='$input' and field = '$field' and glider != '(pending)'");
  print qq("select count(*) as quantity from flight_info where flight_date ='$input' and field = '$field' and glider != '(pending)'") if $DEBUG; 
  $get_info->execute();
  while (my $ans = $get_info->fetchrow_hashref()) {
    $answer += $ans->{'quantity'};
    }
  warn "\$answer is $answer so far... " if $DEBUG; 
  my($get_info) = $dbh->prepare("select count(*) as quantity from ops_days where flight_date ='$input' and field = '$field'");
  $get_info->execute();
  #print qq("select count(*) as quantity from ops_days where flight_date ='$input' and field = '$field'") if $DEBUG;
  while (my $ans = $get_info->fetchrow_hashref()) {
    $answer += $ans->{'quantity'};
    }
  warn "\$answer is $answer so far... " if $DEBUG; 
  my($get_info) = $dbh->prepare("select count(*) as quantity from towplane_data where flight_date ='$input' and field = '$field'");
  warn qq("select count(*) as quantity from towplane_data where flight_date ='$input' and field = '$field'") if $DEBUG;
  $get_info->execute();
  while (my $ans = $get_info->fetchrow_hashref()) {
    $answer += $ans->{'quantity'};
    }
  ...
  if ($answer > 0) {
    ...
    warn_out(qq(It appears that a logsheet for $logsheet{'operations_date'} operating out of $logsheet{'field'} has already been submitted. Should I overwrite what's already in there?<br>\n));
    }
  else {
    print qq(<td bgcolor="#88FF88">OK</td></tr>\n);
    }
...
sub new_contacts {
	# Take input blob, then figure out if the new contacts
	# are multiple or single. Use the extract of ARRAY (if multiple) or HASH (if singular)
  my ($input) = shift;
  if (ref($input) eq "ARRAY") {
	printf (qq(Multiple new contacts extracted: %d<br>\n),
		scalar(@{$input}));
	}
...
sub print_out_staff {
  ...
  $logsheet{'am_towpilot'}    =	find_name($input->{'StaffData_am-towpilot'}->{'PersonName'}->{value});
  $logsheet{'pm_towpilot'}    =	find_name($input->{'StaffData_pm-towpilot'}->{'PersonName'}->{value});
