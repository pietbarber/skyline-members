...
sub insert_member {
  ...
  if (find_name($input->{'ContactInfo_name'}->{'PersonName'}->{'value'}) eq $input->{'ContactInfo_name'}->{'PersonName'}->{'value'}) {
    ...
    if ($input->{'ContactInfo_name'}->{'PersonName'}->{'value'}  =~ /^\s*(\w+)\s+(\w+)\s*$/) {
      $firstname = $1; 
      $lastname = $2; 
     }
    
    if ($input->{'ContactInfo_name'}->{'PersonName'}->{'value'}  =~ /^\s*(\w+)\s+(\w+)\s+(\w+)\s*$/) {
...
# cell_phone     | character varying(16)  | 
	...
	'T', 		# Temporary Member (Introductory Member)
	'', 		# rating
	'', 		# glider #1
	'', 		# glider #2
...
sub process_flight {
  ...
  for my $flight_regime ('takeoff', 'landing') { 
    if ($input->{$flight_regime}->{'ClockTime'}->{'ClockTime_hour'}->{'value'} > 23)   { 
      error_out(qq(One of the flights has a truly bogus value [) . 
        $input->{'FlightInfo_' . $flight_regime}->{'ClockTime'}->{'ClockTime_hour'}->{'value'} .  
	qq(] for the hour field of ) . 
	$input->{'FlightInfo_name'}->{'PersonName'}->{'value'} . 
    ...
    for my $h_m ( 'hour', 'minute') { 
      if ($input->{'FlightInfo_' . $flight_regime}->{'ClockTime'}->{'ClockTime_' . $h_m}->{'value'} == -1) { 
        error_out(qq(One of ) .  
 	 	$input->{'FlightInfo_name'}->{'PersonName'}->{'value'} . 
  ...
  if ($tow_height == -1) {
	...
	# 
  my $towplane = $root->{'Logsheet'}->{'FlightInfo_towplane'}->{'TowPlaneName'}; 
